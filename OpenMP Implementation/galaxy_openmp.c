// This is implemented for OpenMP program.
// the code calculates angles and incrementing the histograms

// In exercise 1, redesign the template for OpenMP: galaxy_openmp.c

// Compilation on dione:
//    module load gcc               // do that once when each log in
...
//  and so on until     89.75 <=  angle  <= 90.0
    ...
    #pragma omp parallel for private(i,j,angle) shared(histogram_DD)
    for(i=0;i<100000L;i++)
    {
        for(j=0;j<100000L;j++)
        {
    ...
    #pragma omp parallel for private(i,j,angle) shared(histogram_DR)
    for(i=0;i<100000L;i++)
    {
        for(j=0;j<100000L;j++)
        {
    ...
    #pragma omp parallel for private(i,j,angle) shared(histogram_RR)
    for(i=0;i<100000L;i++)
    {
        for(j=0;j<100000L;j++)
        {
...
//       {printf("   Histogram sums should be 10000000000. Ending program prematurely\n");return(0);}
    ...
    for ( i = 0; i < 360; ++i )
        if ( histogram_RR[i] != 0L )
           ...
           if ( i < 360 ) printf("      angle %.2f deg. -> %.2f deg. : %.3f\n", i*0.25, (i+1)*0.25, omega[i]);
           }
